...
<div class="container-fluid mt-5">
    <div class="row">
        ...
        <div class="col-lg-8">
                ...
                <div class="card-body">
                    <div class="toggle-section">
                        <label class="custom-toggle">
                            <input type="checkbox" id="ativarLeveMais" checked>
                            <span class="toggle-slider"></span>
                        </label>
                        <span class="toggle-status bg-success">SIM</span>
                        <span class="toggle-label">Ativar Leve mais e pague menos</span>
                    </div>
                    ...
                    <div class="form-group">
                        <label for="valorMinimo" class="form-label">Valor mínimo do produto <i class="ti ti-info-circle text-muted ms-1" data-bs-toggle="tooltip" title="Valor mínimo que o produto deve ter para participar da promoção"></i></label>
                        
                        <div class="mb-3">
                            <div class="toggle-section">
                                <label class="custom-toggle">
                                    <input type="checkbox" id="ativarValorMinimo" checked>
                                    <span class="toggle-slider"></span>
                                </label>
                                <span class="toggle-status bg-success">SIM</span>
                            </div>
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" id="valorMinimo" placeholder="00,00">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Card: Regras Leve Mais e Pague Menos -->
            <div class="card mb-4">
                <div class="card-header bg-purple bg-opacity-10">
                    <h6 class="card-title mb-0 text-purple">
                        <i class="ti ti-discount-2 text-primary me-2"></i>
                        Regras Leve Mais e Pague Menos
                    </h6>
                </div>
                <div class="card-body">
                    <div class="leve-mais-container p-4 bg-light rounded-3">
                        <div class="d-flex align-items-center mb-4">
                            <h6 class="text-purple mb-0 me-2">QUANDO O CLIENTE COMPRAR</h6>
                            <div class="ms-2">
                                <input type="number" class="form-control form-control-sm" id="quantidadeCompra" value="3" min="1" style="width: 70px; text-align: center;">
                            </div>
                            <h6 class="text-purple mb-0 ms-2">ITENS</h6>
                        </div>
                        
                        <div class="d-flex align-items-center mb-3">
                            <h6 class="text-purple mb-0 me-2">O CLIENTE GANHA 100% DE DESCONTO EM</h6>
                            <div class="ms-2">
                                <input type="number" class="form-control form-control-sm" id="quantidadeDesconto" value="1" min="1" style="width: 70px; text-align: center;">
                            </div>
                            <h6 class="text-purple mb-0 ms-2">ITEM(S)</h6>
                        </div>
                        
                        <div class="alert alert-light border-0 mb-0">
                            <p class="text-purple mb-0"><strong>*O DESCONTO DE 100% É APLICADO NOS ITENS DE MENOR VALOR</strong></p>
                        </div>
                    </div>
                </div>
            </div>
...
function salvarDesconto() {
    ...
    const dados = {
        ...
        ativarLeveMais: $('#ativarLeveMais').is(':checked'),
        tituloPromocao: $('#tituloPromocao').val(),
        ativarValorMinimo: $('#ativarValorMinimo').is(':checked'),
        valorMinimo: $('#valorMinimo').val() || null,
        quantidadeCompra: $('#quantidadeCompra').val() || 3,
        quantidadeDesconto: $('#quantidadeDesconto').val() || 1,
    ...
    // Remover validações de campos que não existem mais
    // (descricaoCupom, porcentagemDesconto, valorFixoDesconto)
    // Mantém apenas as validações dos campos realmente presentes no formulário
    if (dados.ativarLeveMais && (!dados.tituloPromocao || !dados.tituloPromocao.trim())) {
        Swal.fire({
            icon: 'warning',
    ...
    if (dados.ativarLeveMais && dados.ativarValorMinimo && (!dados.valorMinimo || dados.valorMinimo <= 0)) {
        Swal.fire({
            icon: 'warning',
    ...
    if (dados.ativarLeveMais) {
        if (!dados.quantidadeCompra || dados.quantidadeCompra < 1) {
            Swal.fire({
                icon: 'warning',
                title: 'Quantidade inválida',
                text: 'Por favor, informe uma quantidade válida de itens para compra (mínimo 1).',
                confirmButtonColor: '#fb6c2b'
            });
            $('#quantidadeCompra').focus();
            return;
        }
        if (!dados.quantidadeDesconto || dados.quantidadeDesconto < 1) {
            Swal.fire({
                icon: 'warning',
                title: 'Quantidade inválida',
                text: 'Por favor, informe uma quantidade válida de itens para desconto (mínimo 1).',
                confirmButtonColor: '#fb6c2b'
            });
            $('#quantidadeDesconto').focus();
            return;
        }
        if (parseInt(dados.quantidadeDesconto) >= parseInt(dados.quantidadeCompra)) {
            Swal.fire({
                icon: 'warning',
                title: 'Configuração inválida',
                text: 'A quantidade de itens com desconto não pode ser igual ou maior que a quantidade de itens para compra.',
                confirmButtonColor: '#fb6c2b'
            });
            $('#quantidadeDesconto').focus();
            return;
    ...
    $.ajax({
        url: '/leve-mais-pague-menos/salvar',
        method: 'POST',
