...
<div class="container-fluid mt-5">
    <div class="row">
            ...
            <div class="card h-100">
                <div class="card-header border-0 pb-0">
                    <h6 class="card-title mb-0">
                        ...
                        Informações do Desconto Progressivo
                    </h6>
                </div>
                <div class="card-body">
                    <p class="text-muted mb-3">Cadastre as informações e configurações do Desconto Progressivo</p>
                    
                    <div class="alert alert-primary border-0" role="alert">
                        <i class="ti ti-diamond me-2"></i>
                        <strong>Resumo:</strong>
                        <div class="mt-2" id="resumoConfig">
                            <small class="text-muted">Configure os campos ao lado para ver o resumo</small>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-8">
            ...
            <!-- Card: Regras de Desconto Progressivo -->
            <div class="card mb-4">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <h6 class="card-title mb-0">
                        ...
                        <button type="button" class="btn btn-sm btn-outline-primary" onclick="adicionarRegra()">
                            <i class="ti ti-plus me-1"></i> Adicionar Regra
                        </button>
                    </div>
                </div>
                <div class="card-body">
                    <div id="regrasDesconto">
                        <!-- Regras serão adicionadas aqui -->
                    </div>
                    
                    <div class="alert alert-info mt-3" id="infoRegras">
                        <i class="ti ti-info-circle me-2"></i>
                        <small>Clique em "Adicionar Regra" para criar taxas de desconto oferecidas de acordo com a quantidade de itens adicionados ao carrinho</small>
                    </div>
                </div>
            </div>
...
$(document).ready(function() {
    ...
    $('#tituloPromocao, #valorMinimo').on('input', updateResumo);
    
    // Função para atualizar resumo
    function updateResumo() {
        const ativo = $('#ativarDesconto').is(':checked');
        const titulo = $('#tituloPromocao').val();
        ...
        if (!ativo) {
            resumoHtml = '<span class="badge bg-danger">Desconto Inativo</span>';
        } else {
            resumoHtml += '<div class="mb-2">';
            
            if (titulo) {
                resumoHtml += `<strong>Título:</strong> ${titulo}<br>`;
            }
            
            if (valorMinimo) {
                resumoHtml += `<strong>Valor mínimo:</strong> R$ ${valorMinimo}<br>`;
            }
            ...
            if (limitarData && dataInicio && dataFim) {
                ...
                resumoHtml += `<strong>Período:</strong> ${inicio} até ${fim}<br>`;
            }
            
            if (estadosSelecionados && estadosSelecionados.length > 0) {
                resumoHtml += `<strong>Estados:</strong> ${estadosSelecionados.length} selecionado${estadosSelecionados.length !== 1 ? 's' : ''}<br>`;
            }
            
            const selectClientes = $('#selectClientes').val();
            if (selectClientes === 'todos') {
                resumoHtml += `<strong>Clientes:</strong> Todos os clientes<br>`;
            } else if (clientesSelecionados && clientesSelecionados.length > 0) {
                resumoHtml += `<strong>Clientes:</strong> ${clientesSelecionados.length} selecionado${clientesSelecionados.length !== 1 ? 's' : ''}<br>`;
            }
            
            if (regrasDesconto && regrasDesconto.length > 0) {
                resumoHtml += `<strong>Regras:</strong> ${regrasDesconto.length} regra${regrasDesconto.length !== 1 ? 's' : ''} configurada${regrasDesconto.length !== 1 ? 's' : ''}<br>`;
            }
            
            const todosProdutos = $('#todosProdutos').is(':checked');
            if (todosProdutos) {
                resumoHtml += `<strong>Produtos:</strong> Todos os produtos<br>`;
            } else if (produtosSelecionados && produtosSelecionados.length > 0) {
                resumoHtml += `<strong>Produtos:</strong> ${produtosSelecionados.length} produto${produtosSelecionados.length !== 1 ? 's' : ''} selecionado${produtosSelecionados.length !== 1 ? 's' : ''}<br>`;
            }
            
            const permitirOutras = $('#permitirOutrasPromocoes').is(':checked');
            if (permitirOutras) {
                resumoHtml += `<strong>Outras promoções:</strong> Permitidas<br>`;
            } else {
                resumoHtml += `<strong>Outras promoções:</strong> Não permitidas<br>`;
            }
            
            resumoHtml += '</div>';
            
            if (!titulo && !valorMinimo && (!estadosSelecionados || estadosSelecionados.length === 0) && selectClientes !== 'todos' && (!clientesSelecionados || clientesSelecionados.length === 0)) {
                resumoHtml = '<small class="text-muted">Configure os campos para ver o resumo</small>';
            }
        }
        
        $('#resumoConfig').html(resumoHtml);
    }
...
// Funções para Regras de Desconto
function adicionarRegra() {
    contadorRegras++;
    const regraId = `regra_${contadorRegras}`;
    
    const regraHtml = `
        <div class="regra-desconto" id="${regraId}">
            <button type="button" class="btn-remove-regra" onclick="removerRegra('${regraId}')" title="Remover regra">
                <i class="ti ti-trash"></i>
            </button>
            
            <div class="regra-title">Desconto Progressivo</div>
            <div class="regra-description">
                Taxas de desconto oferecidas de acordo com a quantidade de itens adicionados ao carrinho
            </div>
            
            <div class="regra-campos">
                <div class="campo-desconto">
                    <label class="form-label">Desconto</label>
                    <div class="input-group">
                        <span class="input-group-text">#</span>
                        <input type="number" class="form-control" placeholder="0" min="0" max="100" id="desconto_${regraId}" onchange="updateResumo()">
                        <span class="input-group-text">%</span>
                    </div>
                </div>
                
                <div class="campo-quantidade">
                    <label class="form-label">Quantidade de itens no carrinho</label>
                    <div class="input-counter">
                        <button type="button" class="counter-btn" onclick="decrementCounter('quantidade_${regraId}')">
                            <i class="ti ti-minus"></i>
                        </button>
                        <input type="number" class="counter-input" value="0" min="0" id="quantidade_${regraId}" onchange="updateResumo()" readonly>
                        <button type="button" class="counter-btn" onclick="incrementCounter('quantidade_${regraId}')">
    ...
    $('#regrasDesconto').append(regraHtml);
    $('#infoRegras').hide();
    
    // Adicionar regra ao array
    regrasDesconto.push({
        id: regraId,
        desconto: 0,
        quantidade: 0
...
function removerRegra(regraId) {
    ...
    // Remover do array
    regrasDesconto = regrasDesconto.filter(regra => regra.id !== regraId);
    
    // Mostrar info se não há regras
    if (regrasDesconto.length === 0) {
        $('#infoRegras').show();
    }
...
function updateRegraData(inputId) {
    const regraId = inputId.split('_')[1] + '_' + inputId.split('_')[2];
    const regra = regrasDesconto.find(r => r.id === regraId);
    
    if (regra) {
        const desconto = $(`#desconto_${regraId}`).val() || 0;
        const quantidade = $(`#quantidade_${regraId}`).val() || 0;
        
        regra.desconto = parseFloat(desconto);
        regra.quantidade = parseInt(quantidade);
    }
...
function salvarDesconto() {
    ...
    const dados = {
        ...
        valorMinimo: $('#valorMinimo').val() || null,
    ...
    if (!dados.titulo.trim()) {
        Swal.fire({
            ...
            text: 'Por favor, informe o título da promoção.',
            confirmButtonColor: '#fb6c2b'
        });
    ...
    if (dados.regrasDesconto.length === 0) {
        Swal.fire({
            icon: 'warning',
            title: 'Regras obrigatórias',
            text: 'Por favor, adicione pelo menos uma regra de desconto.',
            confirmButtonColor: '#fb6c2b'
        });
        return;
    }
    
    // Validar se todas as regras estão preenchidas
    for (let regra of dados.regrasDesconto) {
        if (regra.desconto <= 0 || regra.quantidade <= 0) {
            Swal.fire({
                icon: 'warning',
                title: 'Regras incompletas',
                text: 'Por favor, preencha todos os campos das regras de desconto com valores válidos.',
                confirmButtonColor: '#fb6c2b'
            });
            return;
        }
    ...
    $.ajax({
        url: '{{ route("desconto-progressivo.salvar") }}',
        method: 'POST',
